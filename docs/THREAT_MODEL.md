...
| Stolen or replayed owner dashboard session | Sessions are 32-byte random bearer tokens stored only as SHA-256 hashes, 8-hour lifetime, at most 10 live per owner, revoked by logout, expiry, and owner key rotation. A session may read that owner's snapshot, resolve already-escalated exact approval tuples, close reserved cards, and request payouts to previously verified destinations. `/fund`, `/allocate`, `/mandates`, and `/mandates/:id/revoke` require an owner-signed Ed25519 request; `MONEY_ALLOW_SESSION_OWNER_WRITES=true` relaxes exactly those four routes to session auth for sandbox walkthroughs and is refused by production preflight, so a stolen production session can never move funds into or out of the loop or widen any mandate. The API emits no CORS headers, so a cross-origin page cannot carry the bearer token. | Within its lifetime a stolen production session can still approve a pending escalation (bounded to its exact tuple), close cards, and direct payouts to already-verified destinations. Passkey step-up, device risk, and session-risk ceremonies remain the named launch blockers. |
