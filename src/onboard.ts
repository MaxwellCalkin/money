...
async function main() {
  ...
  const agent = await post<{ id: string }>("/agents", { name: agentName, ownerId: user.id });
  ...
  console.log(`agent   ${agent.id} ("${agentName}") — allocated ${fmt(budget)}`);
  console.log(`mandate ${fmt(budget)} budget · $1/tx · $5/day · ask above $2 · new-payee 10¢`);
  console.log();
  console.log("Paste into .mcp.json:");
  console.log(
    JSON.stringify(
      {
        mcpServers: {
          money: {
            ...
            env: { MONEY_API: API, MONEY_AGENT_ID: agent.id },
          },
        },
      },
      null,
